...
X_cov <- cbind(rexp(200), runif(200))

y <- 2 + X_cov%*%c(-1,1) + u + sigma.e * rnorm(n_obs)
...
df_graph <- data.frame(y = y, edge_number = obs_loc[,1],
                      distance_on_edge = obs_loc[,2], cov_1 = X_cov[,1], cov_2= X_cov[,2])
...
```{r}
res <- graph_lme(y ~ cov_1 + cov_2, graph = graph_bru, model = list(type = "isocov", cov_function = "exp_covariance"))
```

```{r}
data_tmp <- df_graph[round(200*runif(30)+1),]
predict(res, data_tmp, compute_var = TRUE, posterior_sample = FALSE, normalized = TRUE)
```
...
y_rep <- 2 + matrix(rep(X_cov%*%c(-1,1),30),ncol=30) + u_rep + sigma_e * matrix(rnorm(n_obs_rep * n_repl),
                                    ncol=n_repl)
```
...
```{r}
res <- graph_lme(y ~ cov_1 + cov_2, graph = graph_bru)

res <- graph_lme(y ~ -1, graph = graph_bru)

pred <- predict(res, dl_rep_graph, normalized = TRUE)
```
