...
nc <- 1:length(graph$CoB$S)
Qtilde <- Qtilde[-nc,-nc]
R <- Cholesky(Qtilde,LDL = FALSE, perm = TRUE)
V0 <- as.vector(solve(R, solve(R,rnorm(7), system = 'Lt')
                      , system = 'Pt'))
#print(round(t(graph$CoB$T[-nc,])%*%solve(Qtilde)%*%(graph$CoB$T[-nc,]),2))
u_e <- t(graph$CoB$T)%*%c(rep(0,length(graph$CoB$S)),V0)
X <- c()
...
Sigma.overdetermined  = t(graph$CoB$T[-n.c,])%*%solve(Qtilde)%*%(graph$CoB$T[-n.c,])
