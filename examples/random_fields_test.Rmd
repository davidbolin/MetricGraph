...
Let us now generate some observation locations and sample the process: 
...
# graph$add_mesh_observations(rep(NA,nrow(graph$mesh$PtE)))
# graph$observation_to_vertex()

u <- sample_spde(kappa = kappa, sigma = sigma, alpha = 2,
                 graph = graph, type="mesh", BC=0)
...
graph$add_mesh_observations(y)

# graph$add_PtE_observations(y,graph$PtE, normalized = TRUE)
graph$plot(data = TRUE)
graph$observation_to_vertex()
```

Now let us fit the different models to this data: 
```{r}
theta0 <- theta
#alpha = 1 model
