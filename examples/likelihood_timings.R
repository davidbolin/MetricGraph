...
N <- 5
if(graph.type == 1){
  n.obs <- 4*c(50, 100, 150, 200, 250, 300, 350, 400, 450, 500)
} else {
  n.obs <- c(50, 100, 150, 200, 250, 300, 350, 400, 450, 500)
}
...
plot(nE*n.obs,results$v1,col=1,
     type="l",
     ylim = c(0,max(c(max(results$v1),max(results$v2),max(results$co)))),
     xlab = "n",
...
lines(nE*n.obs,results$co, col = 3,lwd = 2)
points(nE*n.obs,results$co,col=3, pch=5)
legend("topleft", legend = c("Version 1",
                             "Version 2",
                             "Covariance-based"),
