...
```{r}
  ...
  graph$plot_function_mesh(c_cov, flat = FALSE)
```
...
```{r, fig.show='hold',fig.align = "center",echo=TRUE}
  graph$plot_function_mesh(as.vector(u.krig[[1]]), flat = FALSE)  
```
...
we will use the `plot_function()` graph method. This
requires us to also provide the node locations on the
graph, so we will bind the locations with the means
of the predictions:
```{r}
m.prd_matrix <- cbind(graph$mesh$PtE,m.prd)
```
We can now plot the predictions:
```{r, fig.show='hold',fig.align = "center",echo=TRUE}
  graph$plot_function(m.prd_matrix, flat = FALSE)  
```
...
of the predictions:
```{r}
  bru_prd_matrix <- cbind(pred_loc, field_pred$mean)
```
We can now plot the predictions:
```{r, fig.show='hold',fig.align = "center",echo=TRUE}
  graph$plot_function(bru_prd_matrix, flat = FALSE)  
```
