...
We will need to supply the `data.frame`. So let us build it:
...
However, it is important to observe that `plot_function` assumes continuity of the function. Let us see an example of a discontinuous function, and observe the effect of `plot_function`. In such a case, `improve_plot` does not affect the action on the discontinuity points.
