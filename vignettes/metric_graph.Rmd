...
following three edges. We will set `perform_merges` to `TRUE`, to show how they are merged with the default choice of tolerances.
```{r, fig.show='hold',fig.align = "center",echo=TRUE, message=FALSE}
edge1 <- rbind(c(0,0),c(1,0))
...
with `perform_merges` set to `FALSE`. To also skip this merge, we can also set `merge_close_vertices` to `FALSE`:
...
download OSM data with the `fetch_osm()` helper.
In the following code we extract highways for a part of the city of
Copenhagen:
```{r, eval=FALSE}
...
```{r, include=FALSE}
# Same call as above, but reading a saved Overpass response so that building
# the vignette does not depend on the Overpass API being available.
bbox <- c(12.5900, 55.6000, 12.6850, 55.6450)
data_sf <- fetch_osm(bbox = bbox,
                     value = c("motorway", "primary",
                               "secondary", "tertiary"),
                     cache_path = "osm_data/copenhagen.osm")
```
...
Alternatively, and more conveniently, whenever the `sf` object has a valid `crs`, it will automatically use this `crs` to set the
correct coordinate reference system (CRS) in the metric graph. Above, we ended up removing the `crs` when we created the `linestring` object from the resulting `osm_lines`. Let us, instead, build the graph directly using the `osmdata_sf` object:
...
in a single call with `metric_graph_from_osm()`:
...
This also allows us to plot the graph interactively with the underlying world map, if the data has a coordinate reference system, by setting `type = "mapview"` in the
`plot()` method:
...
set the argument `check_connected = FALSE` while creating the graph. In this
case the check is skipped and no warning is printed; the resulting
`metric_graph` works exactly as a connected one (see [Working with disconnected
metric graphs](graph_components.html) for a focused walkthrough).

To inspect the components we can use `get_components()`, which returns a list
of `metric_graph` objects sorted by total edge length:
```{r}
...
If we want to keep only the largest connected component for further analysis,
we can take the first entry of `get_components()` (components are sorted by
...
Alternatively, we can set `type = "plotly"` in the plot command to get a 3D 
