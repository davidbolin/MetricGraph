...
Let us obtain the characteristics of this graph:

```{r}
graph5$compute_characteristics()
```

Let us now check the characteristics:

```{r}
graph5
```

We have that this graph does not have Euclidean edges. Let us also check if it satisfies the distance consistency assumption:

```{r}
graph5$check_distance_consistency()
graph5
```
