...
specify a `replicates` argument that it useful for specifying that data is observed at different time points. As an example, let us add two variables observed at two
separate locations 
```{r}
...
graph$add_observations(data = df_rep, replicates = "time")
```
If `NA` is given in some variable, this indicates that this specific variable is
not measured at that location and replicate. 
We can now plot the individual variables by specifying their names in the 
plot function together with the replicate number that we want to see. By default
the first replicate is shown. 
```{r}
...
graph$plot(data = "y", repl = 1)
```
One can note that the command adds all observation locations, from all replicates
as vertices. 
