...
stk.dat <- inla.stack(data = graph$data, 
                        A = A, 
                        effects = c(
      spde_index,
      list(Intercept = 1)
...
m_prd <- cbind(graph$data[["__edge_number"]][idx_prd],
                    graph$data[["__distance_on_edge"]][idx_prd],
                    m_prd)
...
```{r, fig.show='hold',fig.align = "center",echo=TRUE, fig.width=7, warning=FALSE}
graph_prd$add_PtE_observations(data_frame = df_graph, 
                            normalized=TRUE)
```
...
graph_prd$add_mesh_observations(data_frame = data_mesh2)
```
...
dl_graph <- lapply(1:ncol(y_rep), function(i){data.frame(y = y_rep[,i],
                                          ...
                                          distance_on_edge = obs_loc[,2])})
graph_rep$add_PtE_observations(data_list = dl_graph, normalized=TRUE)
