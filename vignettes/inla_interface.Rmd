...
Now, we will create the new data object with the `graph_data_spde()` function, in which we need to provide a name for the random effect, which we will call `field`:

```{r}
data_spde_alpha2 <- graph_data_spde(graph_spde = spde_model_alpha2, name = "field")
```
...
```{r, message=FALSE, warning=FALSE}
spde_fit_alpha2 <- inla(f.s.2, data = data_stk2, control.predictor=list(A=inla.stack.A(stk_dat2)))
```
