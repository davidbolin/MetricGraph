...
Let us generate a dataset to be used throughout this section and add this to the metric graph object. Observe that we are replacing the existing data by setting `clear_obs` to `TRUE`:
...
Finally, we can also apply the `mutate()` function the result of the `get_data()` method:

```{r}
graph$get_data() %>% mutate(new_y = y+y2, y3=ifelse(y>1,1,-1))
```
