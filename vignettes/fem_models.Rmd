...
```{r}
fit <- graph_lme(y ~ x1 + x2, graph = graph, model = "wm1", parallel=TRUE)
```
...
fit2 <- rspde_lme(y ~ -1, data = df_data, loc = c("edge_number", "distance_on_edge"), model = op_est, parallel=TRUE) 
...
graph$add_observations(data = df_data, 
                          normalized = TRUE)
rspde_model <- rspde.metric_graph(graph, parameterization = "spde")
```
