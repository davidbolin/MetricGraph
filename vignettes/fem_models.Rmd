...
We generate the observations as $Y_i = 1 + 2x_{i1} - 3 x_{i2} u(s_i) + \varepsilon_i$, where $\varepsilon_i \sim N(0,\sigma_e^2)$ is Gaussian measurement noise, $x_1$ and $x_2$ are covariates generated by uniform and exponential distributions, respectively. 
```{r}
    sigma.e <- 0.1
...
Let us now fit the model. To this end we will use the `rspde_lme()` function from the `rSPDE` package. To this end, we will also create a new object to obtain the estimation (since the function `rspde_lme()` uses the parameters from the object as initial values, and if they are not supplied, an initial guess is given).

```{r}
op_est <- matern.operators(m = rspde.order, graph = graph)
```

Let us now assemble the `data.frame()` with the observations, the observation locations and the covariates:
```{r}
...
```{r}    
print(data.frame(sigma = c(sigma, fit$coeff$random_effects[2]), 
...
```{r}
  u.krig <- predict(op, A = A, Aprd = Matrix::Diagonal(op$sizeC), Y = Y, sigma.e = sigma.e)
```
...
```{r, fig.show='hold',fig.align = "center",echo=TRUE}
  graph$plot_function(as.vector(u.krig[[1]]), plotly = TRUE)  
```
...
obs.loc_repl <- cbind(rep(obs.loc[,1], n.rep), 
                        rep(obs.loc[,2], n.rep))

df_data_repl <- data.frame(y = y_vec, loc = obs.loc_repl)
```

We can now fit the model in the same way as before by using the `rspde_lme()` function. Note that we can optimize in parallel by setting `parallel` to `TRUE`.

```{r}
fit_repl <- rspde_lme(y_vec ~ -1, model = op_est, repl = repl, 
      data = df_data_repl, loc = obs.loc_repl, parallel = TRUE)
```
...
```{r}    
print(data.frame(sigma = c(sigma, fit_repl$coeff$random_effects[2]), 
                   range = c(r, fit_repl$coeff$random_effects[3]),
...
```{r}
  field_pred <- predict(rspde_model, cmp, rspde_bru_fit, data=data_prd_list, ~field)
```
