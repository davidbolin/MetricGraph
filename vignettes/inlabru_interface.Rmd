...
graph_bru$add_PtE_observations(data_frame = df_graph, normalized=TRUE)
...
graph_bru$add_mesh_observations(data_frame = df_mesh)
graph_bru$observation_to_vertex()
```
...
```{r, fig.show='hold',fig.align = "center",echo=TRUE, fig.width=7, warning=FALSE}
graph_bru_prd$add_PtE_observations(data_frame = df_graph,
                            normalized=TRUE)
```
...
y_prd <- rep(NA, n_obs_mesh)
df_mesh2 <- data.frame(y = y_prd)
graph_bru_prd$add_mesh_observations(data_frame = df_mesh2)
```

Finally, we turn these observations to vertices:
```{r fig.show='hold',fig.align = "center",echo=TRUE, fig.width=7, warning=FALSE}
graph_bru_prd$observation_to_vertex()
graph_bru_prd$plot(data="y")
```
...
dl_rep_graph <- lapply(1:ncol(y_rep), function(i){data.frame(y = y_rep[,i],
                                          ...
                                          distance_on_edge = obs_loc[,2])})
graph_bru_rep$add_PtE_observations(data_list = dl_rep_graph, normalized=TRUE)
graph_bru_rep$observation_to_vertex()
graph_bru_rep$plot(data="y")
graph_bru_rep$plot(data="y", repl=2)
```
