...
```{r}
spde_bru_fit <-
    bru(cmp, data=graph_data_spde(spde_model_bru))
```
...
Now, we pass the spde model object, along with 
our fitted object and the positions
(the positions can be either Euclidean coordinates, 
...
spde_bru_fit_prd <-
    bru(cmp_prd, data=graph_data_spde(spde_model_bru_prd),
      options = list(
        control.mode = list(theta = spde_bru_fit$mode$theta,
                          fixed = TRUE)
      ))
```
