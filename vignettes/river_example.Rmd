...
model.wm1 <- graph_lme(Summer_mn ~ SLOPE + as.factor(netID), 
                          graph = graph, model = 'wm1')
```
and then for `alpha=2`:
```{r, message=FALSE}
model.wm2 <- graph_lme(Summer_mn ~   SLOPE + as.factor(netID), 
...
res.wm1.dir2 <- graph_lme(Summer_mn ~  SLOPE + as.factor(netID), 
                                    graph = graph, model = 'wmd1')
```
