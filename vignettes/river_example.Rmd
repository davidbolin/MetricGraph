...
```{r}
graph$add_observations(graph$mutate(netID = netID), clear_obs=TRUE)
```
...
model.wm1 <- graph_lme(Summer_mn ~ ELEV_DEM + SLOPE + netID, 
                          graph = graph, model = 'wm1')
```
and then for `alpha=2`:
```{r, message=FALSE}
model.wm2 <- graph_lme(Summer_mn ~ ELEV_DEM + SLOPE + netID, 
...
res.wm1.dir2 <- graph_lme(Summer_mn ~ ELEV_DEM + SLOPE + netID, 
                                    graph = graph, model = 'wmd1')
```
