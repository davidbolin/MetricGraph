...
To perform kriging prediction to other locations, one can use the `compute_resdist()` function with an argument that is the matrix of the observation locations and prediction locations, and then use the standard formulat for the conditional expectation of a multivariate normal. For example, suppose that we want to predict the value of the process at the midpoint of the first edge in the graph:
```{r}
locs <- rbind(c(1,0.5), graph$PtE)
D <- graph$compute_resdist(locs, normalized = TRUE)
Sigma <- exp_covariance(D, theta.exp[2:3])
Sigma.o <- Sigma[-1,-1]
diag(Sigma.o) <- diag(Sigma.o) + theta.exp[1]^2

mu.p <- Sigma[1,-1]%*%solve(Sigma.o, graph$y)
var.p <- Sigma[1,1] - Sigma[1,-1]%*%solve(Sigma.o, Sigma[-1,1])
print(c(mu.p, sqrt(var.p)))
```
...
above with this model. The first thing we then need to do is to add all observation locations as vertices in the graph (since the model is only defined in vertices), and then compute the graph Laplacian on this extended graph. We first clone the original graph and then add the observation locations as vertices in the new graph, so that we have the original graph stored. 
```{r, fig.show='hold',fig.align = "center",fig.width=7,echo=TRUE}
graph_extended <- graph$clone()
graph_extended$observation_to_vertex()
graph_extended$compute_laplacian()
```
...
```{r}
loglik_GL1 <- likelihood_graph_laplacian(graph_extended, 
                                          alpha = 1)
