...
```{r, fig.show='hold',fig.align = "center",echo=TRUE}
pred_u <- predict(res, newdata = data, normalized = TRUE, only_latent = TRUE)
...
```{r, fig.show='hold',fig.align = "center",echo=TRUE}
pred <- predict(res, newdata=data_mesh_pred, normalized=TRUE, only_latent=TRUE)
u_est <- pred$mean
...
```{r, fig.show='hold',fig.align = "center",echo=TRUE}
pred_y <- predict(res, newdata=data_mesh_pred, normalized=TRUE, only_latent=FALSE)
y_est <- pred_y$mean
...
```{r, fig.show='hold',fig.align = "center",echo=TRUE}
u_est_exp <- predict(res_exp, newdata = data, normalized = TRUE, only_latent = TRUE)
...
```{r, fig.align = "center",echo=TRUE}
pred_exp <- predict(res_exp, newdata = data_mesh_pred, 
                    normalized=TRUE, only_latent = TRUE)
u_est_exp <- pred_exp$mean
graph$plot_function(X = u_est_exp, vertex_size = 0, edge_width = 0.5)
```
...
```{r, fig.show='hold',fig.align = "center",echo=TRUE}
pred_u_GL <- predict(res_gl, newdata = data, normalized = TRUE, only_latent = TRUE)
...
```{r, fig.align = "center",echo=TRUE}
pred_GL <- predict(res_gl, newdata = data_mesh_pred, 
                    normalized=TRUE, only_latent = TRUE)
u_est_GL <- pred_GL$mean
graph$plot_function(X = u_est_GL, vertex_size = 0, edge_width = 0.5)
```
...
```{r, fig.show='hold',fig.align = "center",echo=TRUE}
pred_u_GL_full <- predict(res_gl_pred, newdata = data, normalized = TRUE, only_latent = TRUE)
graph$plot(X = pred_u_GL_full$mean - u, X_loc = data[,1:2], vertex_size = 0)
```

Let us now obtain predictions at the desired locations:

```{r, fig.align = "center",echo=TRUE}
pred_GL_full <- predict(res_gl_pred, newdata = data_mesh_pred, 
                    normalized=TRUE, only_latent = TRUE)
u_est_GL_full <- pred_GL_full$mean
graph$plot_function(X = u_est_GL_full, vertex_size = 0, edge_width = 0.5)
```
and here the correct way of obtaining predictions of the observed data at the desired locations: 
```{r, fig.align = "center",echo=TRUE}
...
```{r}
spde_bru_fit <- bru(cmp, data=graph_data_spde(spde_model, loc = "loc")[["data"]])
```
...
graph$plot_function(X = mean_field_prd, vertex_size = 0, edge_width = 0.5)
```

Fin §ally, let us plot predictions of the observed data at the mesh locations:
...
graph$plot_function(X = mean_obs_prd, vertex_size = 0, edge_width = 0.5)
```
