...
graph <- metric_graph$new(lines, longlat = FALSE)
...
```{r, fig.show='hold',fig.align = "center",echo=TRUE}
graphs <- graph_components$new(lines = lines, longlat = FALSE)
...
graph$plot_function(u_est_GL_full, vertex_size = 0, edge_width = 0.5)
```
...
graph$plot_function(y_est_GL_full, vertex_size = 0, edge_width = 0.5)
```
...
```{r}
spde_bru_fit <- bru(cmp, data=graph_data_spde(spde_model, loc = "loc"))
```
...
graph$plot_function(mean_field_prd, vertex_size = 0)
```
...
graph$plot_function(mean_obs_prd, vertex_size = 0)
```
