...
```
Here, we see that some edges are very short, and one might want to merge such vertices in the construction of the graph. This can be done by setting the parameter `tolerance` to some desired value of the minimum distance between vertices. Let
us merge some vertices:

```{r}
# graph <- metric_graph$new(graph$lines, tolerance = list(vertex_vertex = 5/10000, 
#                                                         vertex_line = 5/10000,
#                                                         line_line = 5/10000))
```
...
Now, we can compute the predictions for $y$:
...
We can obtain estimates for the latent field by setting `only_latent` to `TRUE`:
...
```{r, fig.show='hold',fig.align = "center",fig.width=7,echo=TRUE}
u_est_exp <- predict(res, data = data, normalized = TRUE, only_latent = TRUE)
...
<!-- `compute_resdist()` function with an argument that is the matrix of the observation locations and prediction locations, and then use the standard formula for the conditional expectation of a multivariate normal. For example, suppose that we want to predict the value of the process at the midpoint of the first edge in the graph: -->
...
res_GL <- graph_lme(y ~ lon + lat, graph = graph, parameterization = "spde",
                        model = list(type = "graphLaplacian")) #default alpha = 1
...
<!-- (Create warning when predicting outside of the data) -->
