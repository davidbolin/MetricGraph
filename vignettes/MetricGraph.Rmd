...
Basic statistical tasks such as likelihood evaluation and prediction is implemented for these three types of models in `MetricGraph`. Further, the package also contains interfaces to `R-INLA` (REF, package available from \url{http://R-INLA.
org/download/} and `inlabru` (REF) that facilitates using those packages for full Bayesian inference of general Latent Gaussian Models (LGMs) that includes Whittle--Matérn fields on metric graphs. 

The package is available to install from the Comprehensive R Archive
Network (CRAN) at \url{https://CRAN.R-project.org/package=MetricGraph}. A development version of the package is available via the repository \url{https://github.com/davidbolin/MetricGraph}. The development version is updated more frequently, and can easily be installed directly in R as described on the repository homepage. 

The following sections describe the main functionality of the package and summarizes some of the required theory. Section 2 introduces metric graphs and the `metric_graph` class, Section 3 shows how to work with random fields on metric graphs, and Section 4 introduces the `inlabru` interface of the package through an application to real data. For a complete introduction to the functionality of the package, we refer to the Vignettes available on the package homepage \url{https://davidbolin.github.io/MetricGraph/}. In particular, that contains an introduction to the `INLA` interface, the implementation of Whittle--Matérn fields with general smoothness, and further details and examples for all methods in the package. 
...
We can note the warning that the graph is not connected, so let us extract the 
largest connected component to work with
```{r, fig.show='hold',fig.align = "center",fig.width=7,echo=TRUE}
graph <- graph_components(lines = lines, longlat = TRUE, only_largest = TRUE)
...
```
Here, we see that some edges are very short, and one might want to merge such vertices in the construction of the graph. This can be done by setting the parameter `tolerance` to some desired value of the minimum distance between vertices. 
...
graph$plot(data = TRUE, vertex_size = 0)
```

One should note here that one needs to specify `normalized = TRUE` in the function `add_PtE_observations()` to specify that the locations are in normalized distance on the edges. Alternatively, if one has data as `SpatialPoints` objects, these can be added to the graph via the function `add_observations()`. To illustrate this, let us again sample some locations at random on the graph, then use the function `coordinates` to transform those to Longitude Latitude coordinates, which we then use to create a `SpatialPoints` object which we add to the graph:
```{r, fig.show='hold',fig.align = "center",fig.width=7,echo=TRUE}
obs.loc <- cbind(sample(1:graph$nE, n.obs), runif(n.obs))
obs.lonlat <- graph$coodinates(PtE = obs.loc, normalized = TRUE)
obs <- rnorm(n.obs)
...
In the command `build_mesh`, the argument `h` decides the largest spacing between nodes in the mesh. Above we chose that as 100m which is a bit coarse. So let us reduce the value of `h` to 1m and rebuild the mesh:
```{r}
  graph$build_mesh(h = 1/1000)
```

Suppose now that we want to display the function $f(s) = \text{Longitude}(s) - \Latitude(s)$ on this graph. We then first evaluate it on the vertices of the mesh and then use the  function `plot_function` to display it:
```{r, fig.show='hold',fig.align = "center",fig.width=7,echo=TRUE}
lon <- graph$mesh$V[, 1]
lat <- graph$mesh$V[, 2]
f <- lon - lat
...
To do so, we again draw some locations at random, then sample the field, clear the observations that we previously added to the graph, and finally add the sampled values:
```{r, fig.show='hold',fig.align = "center",fig.width=7,echo=TRUE}
n.obs <- 200
obs.loc <- cbind(sample(1:graph$nE, n.obs), runif(n.obs))
sigma <- 2
...
y <- u + sigma_e*rnorm(n.obs)
graph$clear_observations()
graph$add_PtE_observations(u, PtE = obs.loc, normalized = TRUE)
graph$plot(data = TRUE, vertex_size = 0)
```

Here, we generated the observations
$$
...
As an illustration, we can also sample the field at the mesh on the graph as follows:
```{r, fig.show='hold',fig.align = "center",fig.width=7,echo=TRUE}
u <- sample_spde(kappa = kappa, sigma = sigma, alpha = 1,
...
The `MetricGraph` package contains implementations of likelihoods for Whittle--Matérn fields observed under Gaussian measurement  noise. In this section we will illustrate these methods. 
Suppose that we want to estimate the model parameters of a Whittle--Matérn field
$u(s)$ observed under Gaussian measurement noise. We have already added data of this type to the graph, so let us now estimate the model parameters based on this data. 

We can now use the `likelihood_graph_spde` function to obtain the log-likelihood function for the data based on a Whittle--Matérn field. 


model for $u$. This function is actually a function factory and returns
the loglikehood function as a function of the parameter `theta`,
which is given in the order `(sigma.e, sigma, kappa)`. By default,
this function gives the parameters in log scale and returns minus 
the value of the log-likelihood so one can directly supply it to the `optim` function.
Assuming that $\alpha=1$, we optimize this likelihood using 
`optim` as follows:

```{r}
theta0 <- graph_starting_values(graph, model = "alpha1")
loglik_alpha1 <- likelihood_graph_spde(graph, alpha = 1)
res <- optim(log(theta0), loglik_alpha1)
theta.est <- exp(res$par)
sigma_e_est <- theta.est[1]
sigma_est <- theta.est[2]
kappa_est <- theta.est[3]
...
We fit the model to the data by using `optim` in combination with the  function `likelihood_graph_covariance()` which implements, among other models, Gaussian processes with isotropic exponential covariance functions. 
Similarly to the `likelihood_graph_spde()` function, `likelihood_graph_covariance()` is also a function
factory.  By default `likelihood_graph_covariance()` returns a function of the parameters given in log-scale and returns minus the log-likelihood,
so one can directly apply it to the `optim` function. Let us create the likelihood function and 
find the maximum:
```{r}
loglik_isoExp <- likelihood_graph_covariance(graph, 
                                      model = "isoCov", 
                                      cov_function = exp_covariance)
res.exp <- optim(log(theta0), loglik_isoExp)
theta.exp <- exp(res.exp$par)
sigma_e_est <- theta.exp[1]
sigma_est <- theta.exp[2]
kappa_est <- theta.exp[3]
results <- data.frame(sigma_e = c(sigma_e_est),
                      sigma = c(sigma_est),
                      kappa = c(kappa_est),
...
graph$plot(X = u_est_exp - graph$y, X_loc = graph$PtE, vertex_size = 0)
```

To perform kriging prediction to the mesh locations based on this model, one can first use the function `compute_resdist_mesh()` to get the resistance metric between those locations. However, since we in this example have a rather large mesh this takes a lot of time, and we do not illustrate it here. 
...
above with this model. The first thing we then need to do is to add all observation locations as vertices in the graph (since the model is only defined in vertices), and then compute the graph Laplacian on this extended graph:
```{r, fig.show='hold',fig.align = "center",fig.width=7,echo=TRUE}
graph$observation_to_vertex()
graph$compute_laplacian()
```

We can then fit the model to data similarly to how we fit the previous models, with the help of the function factory `likelihood_graph_laplacian()`:
```{r}
loglik_GL1 <- likelihood_graph_laplacian(graph, 
                                          alpha = 1)
res <- optim(log(theta0), loglik_GL1)
