...
```{r}
C <- covariance_alpha1_mesh(c(2, 0.2), range = range, sigma = sigma,
                            graph = graph)
...
```{r}
C <- covariance_alpha2_mesh(c(2, 0.2), range = range, sigma = sigma,
                            graph = graph)
graph$plot_function(C, plotly = TRUE)
```
...
C.wm <- covariance_alpha1_mesh(P,kappa=kappa, tau=1/sigma, graph=graph, scale = TRUE)
