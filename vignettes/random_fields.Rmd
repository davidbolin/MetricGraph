...
graph$plot_function(u_est, plotly = TRUE)
```
...
factory. 
By default `likelihood_graph_covariance()` returns a function of the parameters given in log-scale and returns minus the log-likelihood,
...
of the function factory `likelihood_graph_laplacian()`:
```{r}
...
```{r}
Q <- (kappa^2*Matrix::Diagonal(graph$nV) + graph$Laplacian)/sigma^2
u <- solve(chol(Q), rnorm(graph$nV))
y <- graph$A()%*%u + sigma_e*rnorm(length(graph$PtV))
graph$y <- y

res.exp <- optim(log(theta0), loglik_GL1)
theta.exp <- exp(res.exp$par)
sigma_e_est <- theta.exp[1]
sigma_est <- theta.exp[2]
kappa_est <- theta.exp[3]
...
theta0 <- theta
#alpha = 1 model
loglik_alpha1 <- likelihood_graph_spde(graph,
                                        alpha = 1)
res <- optim(log(theta0), loglik_alpha1)
...
#alpha = 2 model
graph$buildC(2)
loglik_alpha2 <- likelihood_graph_spde(graph,
                                        alpha = 2)
res <- optim(log(theta0), loglik_alpha2)
...
loglik_isoExp <- likelihood_graph_covariance(graph,
                                              model = "isoCov",
                                              cov_function = exp_covariance)
res <- optim(log(theta0), loglik_isoExp)
theta.exp <- exp(res$par)
...
loglik_GL2 <- likelihood_graph_laplacian(graph, 
                                          alpha = 2)

res <- optim(log(theta0), loglik_GL2)
...
```{r}
cv.alpha1 <- posterior_crossvalidation(theta.alpha1, graph, model = "alpha1")
cv.alpha2 <- posterior_crossvalidation(theta.alpha2, graph, model = "alpha2")
cv.exp <- posterior_crossvalidation(theta.exp, graph, model = "isoExp")
cv.GL1 <- posterior_crossvalidation(theta.GL1, graph, model = "GL1")
cv.GL2 <- posterior_crossvalidation(theta.GL2, graph, model = "GL2")
...
cv.alpha1 <- posterior_crossvalidation_covariance(theta.alpha1, graph, model = "alpha1")
cv.alpha2 <- posterior_crossvalidation_covariance(theta.alpha2, graph, model = "alpha2")
cv.exp <- posterior_crossvalidation_covariance(theta.exp, graph, model = "isoExp")
cv.GL1 <- posterior_crossvalidation_covariance(theta.GL1, graph, model = "GL1")
cv.GL2 <- posterior_crossvalidation_covariance(theta.GL2, graph, model = "GL2")
results <- data.frame(rmse = c(cv.alpha1$rmse, cv.alpha2$rmse, cv.exp$rmse,
                               cv.GL1$rmse, cv.GL2$rmse),
                      mae = c(cv.alpha1$mae, cv.alpha2$mae, cv.exp$mae,
                               cv.GL1$mae, cv.GL2$mae),
...
graph$add_observations(data = df_graph, normalized = TRUE, replicates="repl")
```

To plot the first replicate, we can simply call `graph$plot(data=TRUE)`. To plot another replicate, we set the 
argument `repl` to the index of the replicate we want to plot. Let us plot the first and second replicates:

```{r}
graph$plot(data="y")
graph$plot(data="y", repl="y.2")
graph$plot(data="y", repl=2)
```

To fit the model, we simply proceed in an identical manner as for the case without replicates. In this case, 
we will use the function factory `likelihood_graph_spde()`:
...
theta0 <- c(sigma_e_start, sigma_start, kappa_start) 
loglik_alpha1_repl <- likelihood_graph_spde(graph, alpha = 1)
res <- optim(log(theta0), loglik_alpha1_repl)
sigma_e_est <- exp(res$par[1])
sigma_est <- exp(res$par[2])
kappa_est <- exp(res$par[3])
results <- data.frame(sigma_e = c(sigma_e, sigma_e_est),
...
Let us now sample from a Whitlle--Matérn process with
`alpha=2` at the same locations. We will also consider 20 replicates:

```{r}
u_alpha2 <- sample_spde(kappa = kappa, sigma = sigma, alpha = 2,
                 graph = graph, PtE = PtE, nsim = n_repl)

y_alpha2 <- u_alpha2 + sigma_e*matrix(rnorm(n.obs.per.edge * graph$nE * n_repl), ncol = n_repl)
```

We will now clear the observations and add the new ones:

```{r}
graph$clear_observations()
graph$add_PtE_observations(y_alpha2,PtE, normalized = TRUE)
```
...
loglik_alpha2_repl <- likelihood_graph_spde(graph, alpha = 2)
res <- optim(log(theta0), loglik_alpha2_repl)
sigma_e_est <- exp(res$par[1])
sigma_est <- exp(res$par[2])
kappa_est <- exp(res$par[3])
results <- data.frame(sigma_e = c(sigma_e, sigma_e_est),
...
Let us now generate some observation locations at random locations on each edge
and sample the process. Let us sample from a Whitlle--Matérn process on the metric graph with
`alpha=1`. We will include an intercept and covariates. The covariates can be added by using the
`covariates` argument from the `add_PtE_observations()` or from the `add_observations()` functions.
We need to pass the covariates as a list.
...
```{r}
df_graph <- data.frame(y=y, Intercept = X_cov[,1], x1 = X_cov[,2], edge_number = PtE[,1], distance_on_edge=PtE[,2])
graph$add_PtE_observations(data_frame=df_graph, normalized = TRUE)
```

Let us now estimate the parameters. We should suppy the start vector as `c(sigma_e, sigma, kappa, beta_1, beta_2)`, where `beta_1` and `beta_2` are
...
loglik_alpha1_cov <- likelihood_graph_spde(graph, alpha = 1, covariates = TRUE)
res <- optim(c(log_theta0, beta_1_start, beta_2_start), loglik_alpha1_cov)
sigma_e_est <- exp(res$par[1])
sigma_est <- exp(res$par[2])
kappa_est <- exp(res$par[3])
beta_1_est <- res$par[4]
beta_2_est <- res$par[5]
results <- data.frame(sigma_e = c(sigma_e, sigma_e_est),
                      sigma = c(sigma, sigma_est),
                      kappa = c(kappa, kappa_est),
...
Let us consider an example with `alpha=2`:
...
graph$add_PtE_observations(y, PtE, covariates = list(X_cov), normalized = TRUE)
```

Now, we estimate the parameters:

```{r}
theta0 <- graph_starting_values(graph, model="alpha2")
log_theta0 <- log(theta0)
beta_1_start <- 0
beta_2_start <- 0

loglik_alpha1_cov <- likelihood_graph_spde(graph, alpha = 2, covariates = TRUE)
res <- optim(c(log_theta0, beta_1_start, beta_2_start), loglik_alpha1_cov)
sigma_e_est <- exp(res$par[1])
sigma_est <- exp(res$par[2])
kappa_est <- exp(res$par[3])
...
Let us now generate some observation locations at random locations on each edge
and sample the process. Let us sample from a Whitlle--Matérn process on the metric graph with
`alpha=1` and 20 replicates. We will include an intercept and covariates. The covariates can be added by using the
`covariates` argument from the `add_PtE_observations()` or from the `add_observations()` functions.
We are passing the covariates as a list, and observe that we considered a different set of covariates for each replicate.
...
X_cov <- lapply(1:n_repl, function(i){cbind(1, runif(nrow(PtE)))})

y <- NULL
for(i in 1:n_repl){
y_tmp <- X_cov[[i]] %*% beta +  u[,i] + sigma_e*rnorm(n.obs.per.edge * graph$nE)
y <- cbind(y, y_tmp)
}


graph$add_observations(y, PtE, covariates = X_cov, normalized = TRUE)
```
...
We can also pass a list with a single set of covariates:
...
Then, we proceed as usual to fit the model:
