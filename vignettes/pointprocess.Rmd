...
- Precomputation becomes beneficial when fitting multiple models (typically 3 or more) with the same spatial structure
- For 3 fits, typical speedups range from 1.5x to 3x depending on model complexity  
- The more models you fit, the greater the relative benefit of precomputation
- Exact SPDE models (from `graph_spde()`) typically show greater speedup benefits than rational SPDE models
- Using `clone_graph = FALSE` provides additional performance improvements when you don't need to preserve the original graph
