...
Now let's do the same comparison for the replicated data. Since the replicates coincide with the ".group" column, we can use the default value for `repl` and `repl_col`.
...
time_precomputed_rep <- system.time({
  ...
  fit2_precomp_rep <- lgcp_graph(y ~ -1 + Intercept + cov_lgcp + f(field, model = spde_model, replicate = field.repl), 
                                 graph = graph, precomputed_data = precomputed_data_rep, verbose = TRUE)
