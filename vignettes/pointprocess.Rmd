...
We will now fit the same model using our `INLA` interface, which simplifies the process of fitting the model. To such an end, we will clear the observations from the graph and add the data as one would obtain it in a real world situation, so without exposure terms.
...
```{r}
inla_fit <- lgcp_graph(y ~ -1 + Intercept + cov_lgcp + f(field, model = rspde_model), graph=graph)
```
...
manual_covariates <- data.frame(Intercept = 1,
                                ...
                                .group = 1)
inla_fit <- lgcp_graph(y ~ -1 + Intercept + cov_lgcp + f(field, model = rspde_model), graph=graph,
               manual_covariates = manual_covariates, interpolate = FALSE)
```
...
```{r}
inla_fit_spde <- lgcp_graph(y ~ -1 + Intercept + cov_lgcp + f(field, model = spde_model), graph=graph)
```
...
```{r}
  ...
  lgcp_sample <- graph_lgcp_sim(n = n.rep, intercept = -1 + 2*cov_lgcp, sigma = sigma,
                            range = range, alpha = alpha,
                            graph = graph)
```
...
```{r}
  ...
  df_rep <- data.frame(y=rep(1,length(lgcp_sample[[1]]$edge_loc)),
                                         ...
                                         Intercept = 1,
                                         cov_lgcp = lgcp_sample[[1]]$edge_number/max_edge_num,
  ...
  for(i in 2:n.rep){
    df_rep <- rbind(df_rep, data.frame(y=rep(1,length(lgcp_sample[[i]]$edge_loc)),
                                         ...
                                         Intercept = 1,
                                         cov_lgcp = lgcp_sample[[i]]$edge_number/max_edge_num,
...
data_rspde <- graph_data_spde(rspde_model, name = "field", 
                                  repl = ".all", repl_col = "rep", covariates = c("Intercept","cov_lgcp"))

stk <- inla.stack(data = data_rspde[["data"]], 
                  A = data_rspde[["basis"]], 
                  effects = data_rspde[["index"]])

spde_fit <- inla(y ~ -1 + Intercept + cov_lgcp + f(field, model = rspde_model, replicate = field.repl), 
                 family = "poisson", data = inla.stack.data(stk),
                 control.predictor = list(A = inla.stack.A(stk), compute = TRUE),
...
Let us now fit the same model using our `INLA` interface, which simplifies the process of fitting the model. To such an end, we will clear the observations from the graph and add the data as one would obtain it in a real world situation, so without exposure terms.
...
inla_fit <- lgcp_graph(y ~ -1 + Intercept + cov_lgcp + f(field, model = rspde_model, replicate = field.repl), 
