...
time_original <- system.time({
  fit1_orig <- lgcp_graph(y ~ -1 + Intercept + f(field, model = rspde_model), 
                          graph = graph)
  fit2_orig <- lgcp_graph(y ~ -1 + Intercept + cov_lgcp + f(field, model = rspde_model), 
                          graph = graph)
  fit3_orig <- lgcp_graph(y ~ -1 + cov_lgcp + f(field, model = rspde_model), 
                          graph = graph)
})

# Time the precomputed approach (multiple fits)
time_precomputed <- system.time({
  fit1_precomp <- lgcp_graph(y ~ -1 + Intercept + f(field, model = rspde_model), 
                             graph = graph, precomputed_data = precomputed_data)
  fit2_precomp <- lgcp_graph(y ~ -1 + Intercept + cov_lgcp + f(field, model = rspde_model), 
                             graph = graph, precomputed_data = precomputed_data)
  fit3_precomp <- lgcp_graph(y ~ -1 + cov_lgcp + f(field, model = rspde_model), 
                             graph = graph, precomputed_data = precomputed_data)
})
...
time_manual <- system.time({
  fit_manual <- lgcp_graph(y ~ -1 + Intercept + cov_lgcp + f(field, model = rspde_model), 
...
time_with_clone <- system.time({
  fit_clone <- lgcp_graph(y ~ -1 + Intercept + cov_lgcp + f(field, model = rspde_model), 
                         graph = graph, clone_graph = TRUE)
})

time_without_clone <- system.time({
  fit_no_clone <- lgcp_graph(y ~ -1 + Intercept + cov_lgcp + f(field, model = rspde_model), 
...
time_original_rep <- system.time({
  fit1_orig_rep <- lgcp_graph(y ~ -1 + Intercept + f(field, model = rspde_model, replicate = field.repl), 
                              graph = graph)
  fit2_orig_rep <- lgcp_graph(y ~ -1 + Intercept + cov_lgcp + f(field, model = rspde_model, replicate = field.repl), 
                              graph = graph)
  fit3_orig_rep <- lgcp_graph(y ~ -1 + cov_lgcp + f(field, model = rspde_model, replicate = field.repl), 
                              graph = graph)
})

# Time the precomputed approach (multiple fits with replicates)
time_precomputed_rep <- system.time({
  fit1_precomp_rep <- lgcp_graph(y ~ -1 + Intercept + f(field, model = rspde_model, replicate = field.repl), 
                                 graph = graph, precomputed_data = precomputed_data_rep)
  fit2_precomp_rep <- lgcp_graph(y ~ -1 + Intercept + cov_lgcp + f(field, model = rspde_model, replicate = field.repl), 
                                 graph = graph, precomputed_data = precomputed_data_rep)
  fit3_precomp_rep <- lgcp_graph(y ~ -1 + cov_lgcp + f(field, model = rspde_model, replicate = field.repl), 
                                 graph = graph, precomputed_data = precomputed_data_rep)
})
