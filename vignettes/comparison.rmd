...
By observing the QQ plot with confidence bands, we can see that the Gaussianity assumption is reasonable, however, the fit can likely improved by considering as latent fields, non-stationary Gaussian models. However, it is not the goal of this vignette as we are only comparing exact models. Our package has implementation for nonstationary Gaussian models by using finite element approximations.