...
fit_GL2 <- graph_lme(y ~ 1, graph=pems_graph,
            model = list(type = "graphLaplacian", alpha = 2), previous_fit = fit_GL2)
```
...
res <- posterior_crossvalidation(fitted_models_list, factor = 1000)
#collect the results in a table including the log likelihood values:
results <- as.data.frame(res[["scores"]])[c(1,6,5,2,3,4)]
results[,-1] <- results[,-1]/1000
results[,7] <- -c(fit_isoexp$loglik, fit_GL1$loglik, fit_alpha1$loglik,
                  fit_alpha1_bc$loglik, fit_GL2$loglik, fit_alpha2$loglik,
                  fit_alpha2_bc$loglik)
colnames(results)[7] <- "neg. log-lik"
results
```
