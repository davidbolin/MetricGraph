...
Let us also obtain the observed values at these regions. To such an end we will use the `filter()` method:

```{r}
y_region1 <- pems_graph$filter(coordx_lwr1 < .coord_x, .coord_x < coordx_upr1,
                          coordy_lwr1 < .coord_y, .coord_y < coordy_upr1)

y_region2 <- pems_graph$filter(coordx_lwr2 < .coord_x, .coord_x < coordx_upr2,
                          coordy_lwr2 < .coord_y, .coord_y < coordy_upr2)                          
```

We can now obtain the predictions. We will use the `augment()` function to obtain such predictions. We start by obtaining predictions for the Whittle-Matérn models with `alpha=1`:
...
Let plot the predictions with the corresponding observed values. First for the model without boundary correction:

```{r}
library(ggplot2)

p <- pems_graph$plot_function(newdata = pred_alpha1, data = ".fitted", 
                    vertex_size = 0) + xlim(coordx_lwr1, coordx_upr1) + 
