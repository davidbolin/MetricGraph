...
```{r}
st_bru_graph_repl <- rspde.spacetime(mesh_space = graph_rep, 
                                mesh_time = mesh_time, 
                                alpha = 1, beta = 1)
```
...
```{r}
st_data_bru_repl <- graph_data_rspde(st_bru_graph_repl, 
                                  repl = ".all",
                                  repl_col = "repl")
```
...
Here we set `bounded_rho = FALSE` when creating the spatio-temporal operator:

```{r}
# Create the spatio-temporal operator with bounded_rho = FALSE
st_bru_graph_unbounded <- rspde.spacetime(
  mesh_space = graph,
  mesh_time = mesh_time,
  alpha = 1,
  beta = 1,
