...
likelihood_alpha2 <- function(theta, graph, data_name = NULL, manual_y = NULL,
  ...
  ind_repl <- (graph$.__enclos_env__$private$data[["__group"]] %in% u_repl)
  ...
  for(repl_y in 1:length(u_repl)){
      ...
      # y_rep <- graph$.__enclos_env__$private$data[[data_name]][graph$.__enclos_env__$private$data[["__group"]] == u_repl[repl_y]]
      y_rep <- y[graph$.__enclos_env__$private$data[["__group"]] == u_repl[repl_y]]
      #build BSIGMAB
      ...
      if(!is.null(X_cov)){
          ...
          if(n_cov == 0){
            ...
            X_cov_repl <- X_cov[graph$.__enclos_env__$private$data[["__group"]] == u_repl[repl_y], , drop=FALSE]
...
likelihood_alpha1 <- function(theta, graph, data_name = NULL, manual_y = NULL,
  ...
  for(repl_y in 1:length(u_repl)){
    ...
    for (e in obs.edges) {
      ind_repl <- graph$.__enclos_env__$private$data[["__group"]] == u_repl[repl_y]
      obs.id <- PtE[,1] == e
      y_i <- y_resp[ind_repl]
      y_i <- y_i[obs.id]
      idx_na <- is.na(y_i)
...
likelihood_graph_covariance <- function(graph,
  ...
  loglik <- function(theta){
      ...
      for(repl_y in 1:length(u_repl)){
          ...
          v <- y_graph[graph$.__enclos_env__$private$data[["__group"]] == u_repl[repl_y]]
          ...
          if(!is.null(X_cov)){
              ...
              if(n_cov == 0){
                ...
                X_cov_repl <- X_cov[graph$.__enclos_env__$private$data[["__group"]] == u_repl[repl_y], ,
                                    drop=FALSE]
...
likelihood_graph_laplacian <- function(graph, alpha, y_graph, repl,
  ...
  loglik <- function(theta){
    ...
    for(repl_y in 1:length(u_repl)){
      ...
      v <- y_resp[graph$.__enclos_env__$private$data[["__group"]] == u_repl[repl_y]]
      na.obs <- is.na(v)
      A.repl <- A[!na.obs, ]
      v <- v[!na.obs]
      n.o <- length(v)
